...
if DATA_DIR is None:
    _searched = [os.path.normpath(os.path.abspath(d)) for d in _CANDIDATE_DIRS]  # _CANDIDATE_DIRS is fine here
...
def main():
    ...
    for trial_subject in np.unique(subject_ids):
        ...
        # --- Inner StratifiedKFold on the LOSO training set ---
        ...
        # Summarise this LOSO trial (after all folds + all models are done)
        if per_model["Hybrid"]["acc"]:
            trial_acc = np.mean(per_model["Hybrid"]["acc"])
            trial_accuracies.append(trial_acc)
            print(f"\nTrial {trial_subject+1} Mean Hybrid Accuracy: {trial_acc*100:.2f}%")
