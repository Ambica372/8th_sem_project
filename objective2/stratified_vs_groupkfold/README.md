...
📁 Project Structure
8th_sem_new/ │ ├── stage1_data/ # Raw data ├── stage2_preprocessing/ # Cleaning + feature extraction ├── stage3_feature_analysis/ # Feature understanding ├── stage4_models/ # Model training │ ├── mlp/ │ ├── dnn/ │ ├── attention/ │ ├── hybrid/ │ ├── decision_fusion/ │ └── comparison/ │ ├── stage4_pipeline/ # Processed numpy arrays ├── docs/ # Detailed explanations ├── run_models.py # Main script └── README.md

▶️ How to Run
Ensure processed data is available in: processed_data/

Run the training script: python stage4_models/run_models.py
...
python bad_rows = np.isnan(X).any(axis=1) | np.isinf(X).any(axis=1) X = X[~bad_rows]
